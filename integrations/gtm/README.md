...
## Контейнер GTM не блокируется

Автоблокировка ConsentKit **намеренно не трогает сам контейнер** —
`googletagmanager.com/gtm.js?id=GTM-XXXX` грузится всегда, до согласия.

Так устроена модель Google: `gtm.js` — это контейнер, а не трекер. Он ничего не
измеряет и не пишет cookie сам по себе; он лишь доставляет на страницу теги,
которые ждут разрешения. Если заблокировать контейнер, теги внутри вообще не
получат шанса увидеть согласие — сломается и Consent Mode, и триггеры
`ck_consent_*`, ради которых этот пакет и существует.

**Что это означает для владельца контейнера.** Каждый тег внутри GTM обязан
попадать в одну из двух категорий:

1. **Google-теги** (GA4, Google Ads, Floodlight) — ничего делать не нужно, они
   читают Consent Mode автоматически, а ядро пушит `consent default` со всеми
   `denied` ещё при парсинге страницы, до загрузки контейнера.
2. **Все остальные теги** (Meta Pixel, TikTok, Criteo, произвольный Custom HTML)
   — должны висеть на триггерах `ck_consent_analytics` / `ck_consent_marketing` /
   `ck_consent_functional` либо иметь собственные consent checks.

Тег, у которого нет ни того, ни другого, сработает **до согласия** — и это будет
нарушением, которое ConsentKit не сможет предотвратить: со стороны страницы он
неотличим от легитимной работы контейнера. Аудит тегов в контейнере — гигиена на
стороне владельца GTM. Раздел «Проверка» ниже показывает, как убедиться, что до
согласия не стреляет ничего.

> Прямая установка `gtag.js` (без контейнера) — другой случай:
> `googletagmanager.com/gtag/js?id=G-XXXX` это уже сам трекер, и он **блокируется**
> как обычно, несмотря на общий с контейнером домен.

## Можно ли грузить ConsentKit из самого GTM?

Технически да: Custom HTML тег с триггером **Consent Initialization — All Pages**,
внутри — теги `<script>` клиента. Но у этого способа есть цена, и по умолчанию мы
его не рекомендуем:

- **Клиент не будет первым на странице.** Всё, что стоит в HTML вне GTM (пиксели
  в шапке, виджеты чата, скрипты темы), стартует раньше контейнера — и до этих
  трекеров автоблокировка просто не успеет. Патчи `createElement`/`src` ставятся
  в момент выполнения ядра; то, что загрузилось раньше, уже не остановить.
- **Сам контейнер грузится с серверов Google до согласия.** Для строгой трактовки
  «никаких сторонних запросов до согласия» это уже проблема — при том, что
  ConsentKit в этой схеме ещё даже не запустился.
- **Вес.** Клиент целиком — около 230 КБ без сжатия (ядро и интерфейс, как есть,
  без минификации), и он поедет через контейнер на каждой странице, вместо
  нескольких тегов `<script>` в `<head>`.

Практический вывод: грузить ConsentKit из GTM имеет смысл, только когда **HEAD
сайта недоступен для правки** (нет доступа к шаблону, чужая CMS) **и все трекеры
и так живут внутри GTM** — тогда терять нечего. Во всех остальных случаях ставьте
клиент прямо в `<head>`, до контейнера.
...
- **Одно событие на категорию за загрузку.** Тег с триггером на категорию
  ...
  при включённом `integrations.gtmDataLayer` — см. следующий пункт).
- **Событий нет, если `integrations.gtmDataLayer: false`** в конфиге ConsentKit.
- **Переменные `ck_consent.*` зависят только от `integrations.gtmDataLayer`.**
  Событие `ck_consent_update` и триггеры категорий гейтятся одним флагом
  `gtmDataLayer`, а Consent Mode — отдельным флагом `gcm`; они независимы.
  При `gcm: false` переменные продолжают заполняться. (В ядре до v0.3 эти два
  гейта были связаны, и при `gcm: false` переменные оставались `undefined` —
  если вы видите такое поведение, обновите клиент.)

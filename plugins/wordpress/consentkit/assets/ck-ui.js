...
(function () {
  ...
  function buildPanel(cfg) {
    ...
    nodes.panel = p;
    nodes.panelBody = body;                // the one scrolling region, see revealInPanelBody
    root.appendChild(scrim);
    root.appendChild(p);
  }
  ...
  function onPanelKeydown(e) {
    ...
    e.preventDefault();
    /* preventScroll on all three focus calls in this file (here, openPanel and
       closePanel): a consent panel must never move the host page. A plain
       focus() scrolls the focused node into view, and the node it lands on —
       the panel, the next control, or whatever the visitor clicked before —
       can sit anywhere in the page's own scroll box, so opening or closing the
       settings used to be able to leave the visitor somewhere else on the
       site. Browsers that ignore the option simply focus as before. */
    next.focus({ preventScroll: true });
    revealInPanelBody(next);
  }

  /* The other half of preventScroll in the Tab trap. The panel itself is
     overflow:hidden and .ck-panel__body is its only scrolling region (the
     category list, the service switches, «Дополнительно»), so a control
     tabbed to below or above the body's visible part would be focused out of
     sight. Bring it into view by moving THAT element's scrollTop, measured
     with getBoundingClientRect against the body — never scrollIntoView(),
     which walks every scrollable ancestor up to and including the host
     document, the very movement preventScroll exists to stop. Nodes outside
     the body (the close button in the head, the buttons in the foot) are
     always visible and are left alone. */
  function revealInPanelBody(node) {
    var body = nodes.panelBody;
    if (!body || !node || typeof node.getBoundingClientRect !== 'function' ||
        typeof body.getBoundingClientRect !== 'function') return;
    var inside = false;
    for (var n = node.parentNode; n; n = n.parentNode) {
      if (n === body) { inside = true; break; }
    }
    if (!inside) return;
    try {
      var b = body.getBoundingClientRect();
      var r = node.getBoundingClientRect();
      var pad = 8;                          // breathing room, not flush with the edge
      if (r.top < b.top || (r.bottom - r.top) > (b.bottom - b.top)) {
        body.scrollTop = Math.max(0, (body.scrollTop || 0) - (b.top - r.top) - pad);
      } else if (r.bottom > b.bottom) {
        body.scrollTop = (body.scrollTop || 0) + (r.bottom - b.bottom) + pad;
      }
    } catch (e) { /* noop: focus has already moved, only the reveal is lost */ }
  }
  ...
  function openPanel(invoker) {
    ...
    panelOpen = true;
    try { nodes.panel.focus({ preventScroll: true }); } catch (e) {}   // never scroll the host page, see onPanelKeydown
  }
  ...
  function closePanel(skipRestore) {
    ...
    if (wasOpen && !skipRestore && lastFocus && typeof lastFocus.focus === 'function') {
      // Restoring focus must not scroll the page back to the invoker either.
      try { if (lastFocus.isConnected !== false) lastFocus.focus({ preventScroll: true }); } catch (e) {}
    }
    lastFocus = null;
  }

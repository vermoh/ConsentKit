...
(function () {
  ...
  /* SPEC V1.25 §1 — three modes, and the reason there are three rather than two.
     ...
     'auto'  — navigator.language, exactly as since 0.1. It deliberately does
               NOT look at <html lang>, and that is not an oversight: on
               builder-made sites the attribute is routinely wrong (Tilda writes
               one template `lang` onto every page, so a Romanian page announces
               itself as Russian). Teaching `auto` to read it would silently
               change the language on every site already running — including the
               ones that are correct today — so `auto` keeps its meaning.
     'page'  — <html lang> first, then navigator.language, then en. The mode for
               a site with real per-language URLs (/ru/, /ro/), where the page
               itself is the authority: a Romanian page must greet a visitor
               with a Russian browser in Romanian.
     a code  — that language, always.

     `docLang`/`navLang` are arguments so the rule is pure and testable without
     globals; the call sites read document/navigator and pass them in. They are
     still defaulted here because ck-debug.js calls this with two arguments
     (see textsRow there) and must resolve 'page' the same way the banner does.
  ...
  function resolveLang(cfgLang, table, docLang, navLang) {
    var nav = (arguments.length > 3) ? navLang
      : ((typeof navigator !== 'undefined' && (navigator.language || navigator.userLanguage)) || '');
    var doc = (arguments.length > 2) ? docLang
    ...
    if (mode === 'page') {
      // An attribute that is missing, empty or names a language we have no
      // locale for ('xx') is no answer at all — fall through to the browser.
      return matchLang(doc, table) || matchLang(nav, table) || 'en';
    }
    if (!mode || mode === 'auto') {
      return matchLang(nav, table) || 'en';
    }
    return matchLang(mode, table) || 'en';
  }

  // The two globals resolveLang() needs, read HERE rather than inside it, so the
  // rule itself stays pure. Both are wrapped: `document.documentElement` is
  // absent in a worker and `navigator.language` throws in a few hardened
  // embeddings, and neither is worth losing the banner over.
  function docLangNow() {
    try {
  ...
  // The config-reading resolve, in one place: both call sites (signature() and
  // mount()) must read the same two sources, or the banner could remount in a
  // language the signature never noticed changing.
  function resolveCfgLang(cfg, table) {
    return resolveLang(cfg && cfg.language, table, docLangNow(), navLangNow());
  }

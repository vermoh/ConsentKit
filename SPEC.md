...
| Д9 | Автоблок не перехватывает parser-inserted `<script src>` известного трекера (запрос уходит до согласия; ловятся только динамические вставки и ручная разметка). Подтверждено 03.09.2026. Нужно исследование техники перехвата без ручной разметки (CSP-meta? перехват через document.write? Service Worker?) либо честное закрепление ограничения + автоподсказка сканера | ядро | исследование ~день |
